...
function preloadActionSliderVideos() {
  for (var i = 0; i < NUM_VIDEOS; i++) {
    var path = SLIDER_BASE + '/measurements_reconstruction_' + N_ACTIONS[i] + '.webm';
    ...
    interp_videos[i].addEventListener('loadeddata', function() {
      ...
      if (videosLoaded === 1) {
        ...
        if (video.videoHeight && video.videoWidth) {
          ...
          $('#interpolation-image-wrapper').css('height', containerHeight + 'px');
        } else {
...
function syncTwoVideos(fromIndex, toIndex) {
  // Only sync the previous and target video for speed
  if (fromIndex >= 0 && fromIndex < NUM_VIDEOS && 
      toIndex >= 0 && toIndex < NUM_VIDEOS &&
      interp_videos[fromIndex] && interp_videos[toIndex]) {
    var currentTime = interp_videos[fromIndex].currentTime;
    interp_videos[toIndex].currentTime = currentTime;
  }
}
